...
@Configurable
@TeleOp
public class NEWMotorRPMControl extends OpMode {
    ...
    private final Pose redGoal = new Pose(133, 133, Math.toRadians(0));//138 133.5 0
    ...
    double timerUpperValue = 0.005; // amount of time timer is active for
    ...
    public void init() {
        ...
        follower.setStartingPose(startingPose == null ? (new Pose(8.125, 8.875, 0)) : startingPose);  //REDCLOSE: X:119 Y:98.5
        follower.update();
...
//        if (hoodPosition > 1.0) hoodPosition = 1.0;
        ...
        // 3. Set Motor Power
        flywheel.setPower(flywheelspeed(distanceToTarget));
        hoodServo.setPosition(tiltangle(distanceToTarget));
        ...
        telemetry.addData("Target Power", "%.2f", flywheelspeed(distanceToTarget));
        telemetry.addData("Velocity (RPM)", "%.1f", currentRPM);
        telemetry.addData("Velocity (Ticks/Sec)", "%.0f", currentTicksPerSecond);
        telemetry.addData("Hood Position", "%.2f", tiltangle(distanceToTarget));
