...
@Autonomous(name = "Red Close GATE", group = "Red Close")
public class AutoRedClose_GATE extends OpMode {
    ...
    private final Pose startPose = new Pose(120.78, 121.70, Math.toRadians(38)); //888125 115888 123/115.5Start Pose of our robot. 68.5  6.5
    private final Pose score = new Pose(94, 106, Math.toRadians(38.5)); // Scoring Pose of our robot. It is facing the goal at a 135 degree angle.
    //    private final Pose move = new Pose(111.5, 98.5, Math.toRadians(38)); // Highest (First Set) of Artifacts from the Spike Mark.
    private final Pose park = new Pose(86, 108, Math.toRadians(38)); // Middle (Second Set) of Artifacts from the Spike Mark.
    private final Pose pickup1 = new Pose(128, 82.5, Math.toRadians(0)); // 76.5Lowest (Third Set) of Artifacts from the Spike Mark.

    private final Pose gotopickup1 = new Pose(100, 82.5, Math.toRadians(0));

    private final Pose pickup2 = new Pose(137, 57.5, Math.toRadians(0)); // 59.5Lowest (Third Set) of Artifacts from the Spike Mark.

    private final Pose gotopickup2 = new Pose(100, 57.5, Math.toRadians(0));

    private final Pose gohitgate = new Pose(118, 72, Math.toRadians(0));

    private final Pose goscore1 = new Pose(110, 72, Math.toRadians(0));

    private final Pose hitgate = new Pose(125.5, 72, Math.toRadians(0));




    private Path scorePreload;
    private PathChain backup, scorePickup1, goscorePickup1, scorePickup2, goscorePickup2, gate, gotogate, end, gograbPickup1, grabPickup1, gograbPickup2, grabPickup2;
    ...
    scorePreload.setConstantInterpolation(startPose.getHeading()); */
        ...
        scorePickup1 = follower.pathBuilder()
                .addPath(new BezierLine(goscore1, score))
                .setLinearHeadingInterpolation(startPose.getHeading(), score.getHeading())
                .build();
        ...
        gotogate = follower.pathBuilder()
                .addPath(new BezierLine(pickup1, gohitgate))
                .setLinearHeadingInterpolation(pickup1.getHeading(), gohitgate.getHeading())
                .build();

        gate = follower.pathBuilder()
                .addPath(new BezierLine(gohitgate, hitgate))
                .setLinearHeadingInterpolation(gohitgate.getHeading(), hitgate.getHeading())
                .build();

        goscorePickup1 = follower.pathBuilder()
                .addPath(new BezierLine(hitgate, goscore1))
                .setLinearHeadingInterpolation(hitgate.getHeading(), goscore1.getHeading())
                .build();
...
//                .build();
    ...
    public void autonomousPathUpdate() {
        switch (pathState) {
            case 0:
                robot.op.setPower(0.27);
                //robot.spin.setTargetPosition(35);
                robot.tilt.setPosition(0.0);
...
//                    follower.followPath(end);
            ...
            case 81: //gate
                ...
                if(!follower.isBusy() && opmodeTimer.getElapsedTimeSeconds() >= 0.6){
                    robot.ip.setPower(-0.05);
                    opmodeTimer.resetTimer();
                    setPathState(9);
            ...
            case 11:
                ...
                if(!follower.isBusy() && opmodeTimer.getElapsedTimeSeconds() >= 0.5){
                    ...
                    follower.followPath(goscorePickup1);
                    opmodeTimer.resetTimer();
                    setPathState(121);
...
//                break;
            case 21:
                if ((!follower.isBusy()) &&  (opmodeTimer.getElapsedTimeSeconds() >= 1.35)){
                    robot.ip.setPower(0);
                    setPathState(22);
                }
                break;
...
//                    follower.followPath(shake2);
                ...
                break;
            case 27: //shoot 3
                if (opmodeTimer.getElapsedTimeSeconds() >=1.75){
                    robot.lift.setPosition(0.47);
                    opmodeTimer.resetTimer();
                    setPathState(28);
                }
...
//                break;
        ...
        telemetry.addData("Final Heading", "%.2f°", robotHeading);
        telemetry.update();
