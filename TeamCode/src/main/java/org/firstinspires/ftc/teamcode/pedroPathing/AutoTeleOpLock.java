...
@Configurable
@TeleOp
public class AutoTeleOpLock extends OpMode {
    ...
    private final Pose redGoal = new Pose(133,133, Math.toRadians(0)); // 141.75 135.2  lastest #s 140/135 138 133.5 0  142 125 38 test #s
...
//        targetPower2 = flywheelspeedfar(distanceToTarget);
//        hoodPosition2 = tiltangle2(distanceToTarget);
...
//            flywheel.setPower(targetPower2);
//            hoodServo.setPosition(hoodPosition2);
...
//        double tolerance2 = 115;
        ...
        if (gamepad2.left_bumper && !intakeDisabled){
...
/** SPOON (LIFT) **/
            ...
            if (gamepad2.right_bumper && velocityError <= tolerance ) {
                robot.lift.setPosition(0.47);
                timerValue = 0;
                isTimerActive = true;
            }

//            if (gamepad2.right_bumper && velocityError2 <= tolerance2) {
//                robot.lift.setPosition(0.47);
//                timerValue = 0;
//                isTimerActive = true;
//            }
        ...
        double currentRPM = (currentTicksPerSecond / TICKS_PER_REV) * 60.0;
            ...
            telemetry.addLine("CLOSE SHOOTING!");
...
//            telemetry.addLine("FAR SHOOTING!");
//            telemetry.addData("Target Power", "%.2f", targetPower2);
//            telemetry.addData("Velocity (RPM)", "%.1f", currentRPM);
//            telemetry.addData("Velocity (Ticks/Sec)", "%.0f", currentTicksPerSecond);
//            telemetry.addData("Distance To Target", distanceToTarget);
//            telemetry.addData("Velocity Error", velocityError2);
//            telemetry.addData("Target Velocity", targetVelocityTicks2);
//            telemetry.addData("Hood Position", "%.2f", hoodPosition2);


        telemetry.addLine("--- BALL TRACKING ---");
        telemetry.addData("Ball Count", "%d / %d", ballCount, MAX_BALLS);
        telemetry.addData("Beam 1 RAW", beam1Raw);
        telemetry.addData("Beam 2 RAW", beam2Raw);
        telemetry.addData("Beam 3 RAW", beam3Raw);
        telemetry.addData("Raw True Count", rawTrueCount);
        telemetry.addData("Beam 1 (Intake)", beam1Broken ? "BALL" : "EMPTY");
        telemetry.addData("Beam 2 (Middle)", beam2Broken ? "BALL" : "EMPTY");
        telemetry.addData("Beam 3 (Shooter)", beam3Broken ? "BALL" : "EMPTY");
        telemetry.addData("Intake Delay Active", intakeDelayActive ? "YES" : "No");
        telemetry.addData("Intake Delay Timer", "%.3fs / %.1fs", intakeDelayTimer, INTAKE_STOP_DELAY);
        telemetry.addData("Intake Status", (intakeDelayActive && intakeDelayTimer >= INTAKE_STOP_DELAY) ? "DISABLED (Delay Complete)" : "ENABLED");
        telemetry.addData("Empty Timer", "%.2fs / %.2fs", emptyDetectionTimer, EMPTY_CONFIRMATION_DELAY);
        telemetry.addData("Confirmed Empty", confirmedEmpty ? "YES - WILL RUMBLE" : "No");
        telemetry.addData("Rumble Flag", hasRumbledForEmpty);
        telemetry.addData("Ball Count Light", "Position: %.3f", robot.light2.getPosition());
        ...
        telemetry.addData("Heading (Deg)", "%.2fÂ°", robotHeading);
        telemetry.addData("Turrent Position", robot.spin.getCurrentPosition());
        telemetry.addLine("--- TURRET OFFSET ---");
        telemetry.addData("Manual Offset", "%.1f°", turretOffsetDegrees);
        telemetry.addData("Base Angle", "%.1f°", targetAngle);
        telemetry.addData("Adjusted Angle", "%.1f°", adjustedTargetAngle);
        telemetry.addLine("--- AIMING DEBUG ---");
        telemetry.addData("Robot Pos", "(%.1f, %.1f) @ %.1f°",
                currentPose.getX(), currentPose.getY(), robotHeading);
        telemetry.addData("Goal Pos", "(%.1f, %.1f)",
                currentGoal.getX(), currentGoal.getY());
        telemetry.addData("Error", "dX:%.1f dY:%.1f",
                errorPose.getX(), errorPose.getY());
        telemetry.addData("Field Target Angle", "%.1f°", fieldTargetAngle);
...
//        }
    ...
    public static double flywheelspeed(double TD) {
        return MathFunctions.clamp(((1.93938 * Math.pow(10, -7)) * Math.pow(TD, 3)) - (0.0000564908 * Math.pow(TD, 2)) + (0.00705808 * TD) + 0.168781, 0, 1);
    }

    public static double tiltangle(double TD) {
        return MathFunctions.clamp(((6.67204 * Math.pow(10, -8)) * Math.pow(TD, 4)) - (0.000025167 * Math.pow(TD, 3)) + (0.00325085 * Math.pow(TD, 2)) - (0.155043 * TD) + 2.42651, 0, 1);
    }
